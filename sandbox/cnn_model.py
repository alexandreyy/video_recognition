...
class VideoRecognitionCNN:
    ...
    def model(self, X, dropout=1.0, reuse=False, name="test"):
        ...
        CNN model.
        """
        ## Considering the depth of the network as 34

        with tf.variable_scope(name, reuse=reuse):
            name = "conv_1"
            with tf.variable_scope(name):
                conv_1 = self.r2_1d(name, X, [1, 7, 7, self.input_size[3], 64], down_sampling=True)
            ...
            name = "conv_3x"
            with tf.variable_scope(name):
                conv_3x1 = self.resnet_block(name + "1", conv_2x3, 64, 128, down_sampling=True)
            ...
            name = "conv_4x"
            with tf.variable_scope(name):
                conv_4x1 = self.resnet_block(name + "1", conv_3x4, 128, 256, down_sampling=True)
            ...
            name = "conv_5x"
            with tf.variable_scope(name):
                conv_5x1 = self.resnet_block(name + "1", conv_4x5, 256, 512, down_sampling=True)
            ...
            with tf.variable_scope(name):
                ...
                layer_2_fc = self.fc(name, final_avg,
                                     [512, self.label_size])
